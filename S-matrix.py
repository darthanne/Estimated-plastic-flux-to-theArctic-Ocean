...
difs = np.zeros(its)
for i in range(its):
    difs[i] = np.sum(np.absolute(mats[i] - mats[799])) / PTot[i]

plt.imshow(np.roll(np.flipud(np.log10(mats[60].todense())), 180), animated=True,
           norm=colors.BoundaryNorm(np.linspace(-2, 8, 10, endpoint=True), ncolors=256), cmap='plasma')
plt.show()
